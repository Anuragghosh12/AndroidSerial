...
public class TerminalFragment extends Fragment implements ServiceConnection, SerialListener {
    ...
    public void saveTextToFile() {
        String textToSave = receiveText.getText().toString(); // Get the current text from the TextView
...
//        File file = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "SerialData.txt");
//
//
//
//        try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file, true)))) {
//            writer.write(textToSave);  // Write the text from the TextView to the file
//            writer.newLine();  // Optionally add a new line at the end
//
...
//        }
        ...
        try {
            ...
            values.put(MediaStore.MediaColumns.DISPLAY_NAME, "SerialData");       //file name
            values.put(MediaStore.MediaColumns.MIME_TYPE, "text/plain");        //file extension, will automatically add to file
            values.put(MediaStore.MediaColumns.RELATIVE_PATH, Environment.DIRECTORY_DOCUMENTS + "/Serial Data/");     //end "/" is not mandatory
    ...
    private ArrayList<float[]> extractGraphData() {
        ArrayList<float[]> graphData = new ArrayList<>();
        String text = receiveText.getText().toString(); // Get the current text from receiveText
        ...
        for (String line : lines) {
            ...
            if (columns.length >= 4) {
                try {
                    ...
                    for (int i = 0; i < 4; i++) {
                        points[i] = Float.parseFloat(columns[columns.length - 4 + i]); // Extract the last 4 columns
                    }
                    graphData.add(points);
        ...
        System.out.println("Extracted graph data: " + graphData);
        return graphData;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        else if (id == R.id.show_graph) {
            ArrayList<float[]> graphData = extractGraphData(); // Extract data for the graph
            GraphFragment graphFragment = GraphFragment.newInstance(graphData);

            getParentFragmentManager()
                    .beginTransaction()
                    .replace(R.id.fragment, graphFragment, "graph")
                    .addToBackStack(null)
                    .commit();
            return true;
        }
        else if (id == R.id.save_to_text) {
//            if (receivedData.isEmpty()) {
//                Toast.makeText(getContext(), "No data to save", Toast.LENGTH_SHORT).show();
//            } else {
            //ExcelUtil.saveDataToExcel(getContext(), receivedData, "SerialData.xlsx");
                saveTextToFile();
            //}
            return true;
        }
    ...
    private void receive(ArrayDeque<byte[]> datas) {
        ...
        for (byte[] data : datas) {
            // Convert byte array to a string message

            // Append formatted message to the Spannable string (for UI display)
            spn.append(TextUtil.toHexString(data));



        }

        // Append the parsed data to the receiveText field (UI update)
        receiveText.append(spn);
    }
